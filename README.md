...
**Höchstens drei Fotos pro Artikel – und im Bild steht, wie man sie findet.** Ein Artikel hat höchstens drei Fotoplätze: das Aufmacherbild, das zweite Bild im Text und ein weiteres (Nebengeschichte, Anzeige oder Vorschaubild). Weitere Bilder von Modulen werden als grafische Farbfläche gesetzt, wie es Anzeigen und Teaser oft sind; kleine runde Porträts (Autorenzeile, Kommentare) zählen nicht. In jedem Fotoplatz ohne echtes Foto stehen **direkt im Bild** (nur am Bildschirm – nie im Druck, in Word oder im PNG):
- **drei Google-Bildsuchen** – kurze, allgemeine Suchbegriffe mit hoher Trefferwahrscheinlichkeit (Ort, Szene, Alltagsgegenstand; nie ein erfundener Name oder ein erfundenes Ereignis), jede als Link, der Google Bilder in einem neuen Tab öffnet;
- **ein ChatGPT-Prompt** für ein fotorealistisches Foto wie aus einer Kamera (35 mm, Augenhöhe, natürliches Licht, Schärfentiefe, Querformat 3:2, ohne Text, Logos, Wasserzeichen und erkennbare reale Personen) – mit **Prompt kopieren** und **In ChatGPT öffnen**.
...
> **Im veröffentlichten Artifact auf claude.ai ist das nicht möglich.** Die Seite läuft dort unter einer Sicherheitsrichtlinie (CSP), die Abrufe (`fetch`), Bilder und Medien von allen fremden Hosts blockiert – auch Wikimedia und Openverse. Die App erkennt das an der Meldung des Browsers (`securitypolicyviolation`), sucht in dieser Sitzung nicht mehr, setzt ohne Verzögerung das gezeichnete Bild und sagt im Layout-Tab in einem Satz, dass diese Ansicht keine Bilder aus dem Internet laden darf. Wirksam wird die Suche, wo die Seite fremde Hosts erreichen darf (z. B. `app/index.html` lokal im Browser geöffnet). Im Artifact bleibt der Weg über ein eigenes Bild: im Web **Bild kopieren** und im Dialog *Bild ersetzen* mit **Strg+V** einfügen oder eine Datei wählen – das funktioniert dort.

- **Darf hier überhaupt ein echtes Foto stehen?** Claude beurteilt im Layout-Schritt (`photoReality`): *real-subject* – ein realer, allgemeiner Gegenstand (Stadt, Landschaft, Tier, Technik, Alltag) → echtes Foto möglich; *fictional-event* – ein erfundenes Ereignis, eine erfundene Person oder Firma → nur eine **allgemeine Szene**, die nicht als Beleg missverstanden werden kann, Fotos realer Ereignisse (Brand, Unfall, Protest, Wahl …) werden verworfen, und die Bildlegende lautet „Illustrative photo: …“; *none* oder keine Angabe → **das gezeichnete Bild**, gesucht wird nicht.
- **Suchbegriff aus dem Text.** `photoQuery`: vier bis acht englische Wörter aus dem generierten Text, in der Bildsprache des Mediums (Nachrichtenfoto, Ortsaufnahme, Editorial-Foto, Porträt im Umfeld, Labor oder Landschaft). Fehlt er, entsteht er aus Schlagzeile und Bildlegende des generierten Textes – nie aus den Einstellungen.
- **Quelle und Anfrage.** Zuerst Wikimedia Commons (anonymer Browser-Zugriff mit `origin=*`, `formatversion=2`, sauber codiertem Suchbegriff, Standard-Vorschaubreite 1280 px – andere Breiten weist das CDN ab –, Kennung im Header `Api-User-Agent`, bei abgelehntem Preflight dieselbe Anfrage ohne Header), dann mit kürzerem Begriff noch einmal, dann Openverse (anonym; bei Ratenbegrenzung sofort weiter). Keine Schlüssel, keine Server.
- **Auswahl.** Nur JPEG-Fotos (Commons) bzw. Kategorie Foto (Openverse), keine Logos, Karten, Diagramme, Screenshots, Collagen; quer für den Aufmacher, mindestens 640 × 360 px; nur freie Lizenzen, **nie ND**; **fehlt Urheber oder Quellseite** (bei CC BY), wird das Bild verworfen statt ein Nachweis erfunden. Gereiht nach Übereinstimmung mit dem Text.
- **Erst geladen, dann gezählt.** Ein Kandidat zählt erst, wenn die Datei heruntergeladen ist, wirklich ein Bild ist und sich **dekodieren** lässt (mindestens 480 px breit); sonst der nächste Kandidat, dann die nächste Quelle.
- **Eingefroren ins Material.** Das Foto wird verkleinert (JPEG), im Bildspeicher der Seite abgelegt (sonst als kleines JPEG im Material selbst) und über denselben Weg wie ein eigenes Bild gezeichnet – nie wieder von der Quelle geladen. Deshalb ist die Canvas nie „tainted“: Vorschau, Blatt, Viewer, PNG und Word zeigen dasselbe Foto, auch wenn die Quelle später nicht mehr erreichbar ist. Es zählt erst, wenn diese Seite es wirklich anzeigen kann. Die Bildlegende sagt nur, was die Quelle über das Foto sagt (ihr Titel); Urheber, Quelle, Lizenz und Quellseite stehen am Bild und in der Lehrerversion.
- **Wiederöffnen.** Ein gespeichertes Foto, das noch lädt, erscheint als ruhige Fläche – nie kurz als gezeichnete Szene –, und die Seite wird gezeichnet, sobald es da ist (oder, falls es nie kommt, mit der Szene). PNG, HTML und Word warten auf die gespeicherten Fotos.
- **Nie ein Hindernis.** Offline, CSP, CORS, DNS, Zeitüberschreitung (12 s gesamt, 6 s je Anfrage), leeres Ergebnis, kaputtes Bild, abgelehnte Lizenz, Abbrechen: das Material entsteht wie bisher. Ein gestoppter Lauf kann kein Foto mehr in ein späteres Material setzen; ein bereits hochgeladenes Bild wird wieder entfernt. Jeder Ausgang hinterlässt Codes in `material.layout.photoSearch.codes` (z. B. `commons-search-failed`, `image-cors-failed`, `image-decode-failed`, `licence-rejected`, `metadata-incomplete`, `event-photo-rejected`, `csp-blocked`, `timeout`, `cancelled`, `fallback-used`) – für Tests und Fehlersuche; die Lehrperson sieht nur einen Satz im Layout-Tab.
